...
class GRUModule(nn.Module):
    def __init__(self, len_feature, num_words, len_hidden, num_to_word, word_to_num, strategy='gru', pad_id=0, start_id=1, end_id=2, drop_out=0.5):
        # 特征长度，句子长度，隐层宽度，
        ...
        self.gru=nn.GRU(input_size=len_feature, hidden_size=len_hidden, batch_first=True)
    ...
    def forward(self, x):
        ...
        grued, _ = self.gru(dropouted)
        output = self.fc(grued)
        ...
        for head in heads:
            ...
            for i in range(len(heads)):
                ...
                for j in range(max_len-1): # 开始作诗
                    ...
                    grued, (hn, cn) = self.gru(embedded, (hn,cn))
                    output_digit = self.fc(grued)
